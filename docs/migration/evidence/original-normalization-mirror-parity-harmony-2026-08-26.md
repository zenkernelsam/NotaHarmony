...
- Harmony 事实：`OriginalImageNormalizer.ets` 只在降采样或非零四分之一旋转时
  重写 WebP，当前也只调用 `pixelMap.rotate(rotationDegrees)`；阈值内保留原始
  字节，显示端由 `ImageAssetLoader` 应用完整 2/4/5/7 镜像集合。
- 审计结论：这不是 Phase 444 遗漏，而是原版同源限制。生产代码保持不变；
  新增 Replay 锁定“显示补镜像、归一化不补镜像”的边界。
