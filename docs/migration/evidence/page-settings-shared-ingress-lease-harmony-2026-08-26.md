...
## Phase 523 增量（2026-08-26）

- 复核发现自定义颜色的 H/S/V 更新事件只拒绝共享照片导入租约；忙碌、共享纸库加载或间距保存期间，
  晚到滑杆事件仍可改写 HSV 控件并提交草稿颜色。
- 三个通道统一先拒绝 `busy || photoImportLeaseActive || sharedBusy || spacingSaveBusy`。按钮回调、
  共享模板/收藏/间距路径和正常语义不变。
- 页面设置共享租约 Replay 扩展至 `TOTAL=25 FAILED=0`：要求恰好三处组合早退，并禁止旧的“仅租约”
  自定义颜色事件模式。
