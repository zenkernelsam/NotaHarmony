...
### Phase 4 — 备份、导入导出与 WebDAV 同步（差异化特性）

**目标**：实现 .note 包导入导出 + 兼容原版 Notability 导出文件 + WebDAV 云备份。

| 交付物 | 说明 |
|--------|------|
| **4A .note 兼容导入** | 解析原版 Notability 导出的 .note ZIP → 解码 FlatBuffers op 流 → 转为内部模型 → 可编辑 |
| **4B .note 导出** | 内部笔记 → 导出为 .note ZIP（元数据 JSON + op 流二进制 + 资源文件） |
| **4C WebDAV 客户端** | @ohos.net.http + 标准 WebDAV 协议 |
| **4D 自动备份** | 后台任务 + 增量同步 |
| **4E 恢复** | 从 WebDAV 拉取 + 冲突处理 |

**待确认**：
- 原版 .note 样本文件待用户提供（用于逆向 ZIP 内部结构）
- 兼容程度目标：只读打开 vs 编辑后再导出回原版可读格式

**完成标准**：原版 Notability 导出的 .note 可打开并渲染；笔记可导出；WebDAV 自动备份/恢复可演示。
