...
|----------|--------------|----------|
| 页选择集合 + 多文件分享分发 | 仅当前页单文件 | 目录 picker 或打包策略 + 面板页选择 UI（已由 Phase 644 / ADR-0611 补齐全部/当前页二选 + zip 打包；任意子集再由 Phase 670 `7a0bc882` 全闭环——缩略图栅格任选页勾选） |
...
- ~~页范围选择：随面板两步交互一并补齐。~~ 已闭环：Phase 644
  二选子集 → Phase 670 任意子集勾选（v6d.l Set 语义全对齐）。
