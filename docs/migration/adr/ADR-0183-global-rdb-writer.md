...
所有已经开始事务的编辑器快照、页面结构、持久 Undo/Redo、同步 inbox、录音、笔记和文件夹仓储因此进入同一 FIFO 写队列。资产链路仍需要独立 `assetMutationMutex` 保护文件与 `note_asset.note_ids` 的组合语义；锁序固定为：
...
单语句写入也遵守同一写者：`ToolRepositoryImpl` 删除私有 `writeMutex`，四个工具/工具箱保存入口改用 `databaseWriteMutex`；`NoteRepositoryImpl.saveViewState()` 的 `ON_CONFLICT_REPLACE` 同样进入全局写队列。资产删除事务返回已提交的本地路径，释放数据库写者后才执行最佳努力文件删除，但继续持有资产锁。
...
- 文件清理、通知和其他提交后副作用不得无故延长数据库写者持有时间。
...
全局写者会让长事务阻塞其他保存，因此后续仍应审计事务内文件 I/O、过大批次和可移到提交后的工作。图片资产到达目前为了文件/行补偿仍在事务内完成原子文件落位，这是既有设计，不代表所有文件 I/O 都应留在写锁内。真实后台同步、快速翻页、录音到达和应用生命周期仍需设备并发测试。
