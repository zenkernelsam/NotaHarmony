...
`xj2.k(null, createValue)` 建立的 register 只有默认 value，winner 仍为 null；因此首个 MODIFY 必须获胜，即使它的 op ID 小于
CREATE_INK。旧版把 CREATE op ID 当 winner 的结论已由 ADR-0008 和数据库 v30 修正。
...
数据库 v28 为 `original_ink_state` 增加 `center_path_winner_timestamp/site_id`，当时用目标 Ink 身份作无 null 占位。数据库 v30
另加 `center_path_winner_present`，把等于 Ink 身份的历史占位无损恢复为 winner 缺席；后续真实 MODIFY 才写入 winner。
v26 及更旧的无 state Ink 继续明确 DEFERRED，不反推原始压缩路径。
...
- `d02-modify-ink.mjs` 覆盖真实 19-field `wd8`、field 8 布局、v27→v28 历史占位、严格 LWW、多 Ink 单 revision、
  append 保留/重连、live/archive、unsupported-field 门禁、分歧原子性、故障回滚、缺 state 与无本地日志；
  `d02-modify-ink-transform.mjs` 另覆盖 v29→v30 缺席 winner 恢复与较小 op ID 首写获胜。
