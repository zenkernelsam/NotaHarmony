...
Accepted，2026-08-15。本文关闭 ADR-0124 中“语法级 Invalid/Ok 与编辑器公式预览尚未完成”的边界。
...
- `axi.a()` 用 `lwa(Empty)` 创建编辑器状态，并固定把渲染宽高设为 280×96。
...
- `p18.invoke()` 以 1.0 scale 创建完整目标 bitmap，调用 `s18.d()` 做框内 fit，并把主题文字色交给
...
3. 只有 `OriginalMathEngine` 已就绪时才执行完整 280×96、1× render。引擎仍在初始化时保持 Loading；初始化
   完成后自动续验。初始化已结束但引擎不可用时 fail closed 为 Invalid。
...
- Harmony Native Drawing 与 Android Canvas 的字体 fallback、hinting、抗锯齿和密度行为仍可能产生平台像素差异。
