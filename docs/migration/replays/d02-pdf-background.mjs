...
assert.match(loader, /page = document\.getPage\(pdf\.pageInAsset\);[\s\S]*page\.getAreaPixelMap\(matrix/);
assert.match(loader, /visible-area raster failed; using full-page fallback[\s\S]*page\.getPagePixelMap\(\)/);
assert.match(loader, /pageToRelease\.release\(\)[\s\S]*document\.releaseDocument\(\)/);
assert.match(renderer, /drawPdfBackground/);
assert.match(renderer, /pdfRenderRect/);
assert.match(thumbnail, /pdfLoader\.load\(effective\.pdf, rasterRequest\)/);
