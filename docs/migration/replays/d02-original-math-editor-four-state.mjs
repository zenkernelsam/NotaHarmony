...
const checks = [
  ...
  [overlay,
    /return !busy && !photoImportLeaseActive &&\s+state === OriginalMathEditorDraftState\.OK/,
  ...
  [canvas, /ORIGINAL_MATH_EDITOR_PREVIEW_PIXEL_SCALE: number = 1/,
    'Harmony preserves the original post-density 1x editor preview scale'],
  [canvas, /previewWidthPixels: number = vp2px\(ORIGINAL_MATH_EDITOR_PREVIEW_WIDTH_VP\)[\s\S]*?previewHeightPixels: number = vp2px\(ORIGINAL_MATH_EDITOR_PREVIEW_HEIGHT_VP\)/,
    'Harmony converts the original logical preview box to physical pixels'],
  [canvas, /mathEditorValidationGeneration[\s\S]*?setTimeout\(\(\): void =>/,
    'Harmony defers and generations native validation'],
  [canvas, /originalMathEngine\.render\(draft,[\s\S]*?previewWidthPixels, previewHeightPixels/,
