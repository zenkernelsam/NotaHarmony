...
assert.match(renderer, /orientationTransform: TransformMatrix/);
assert.match(renderer, /originalImageOrientationTransform\(/);
assert.match(renderer, /const mirror: TransformMatrix = mirroredHorizontally \?/);
assert.match(renderer, /return multiplyTransform\(rotation, mirror\);/);
assert.ok(renderer.indexOf('ctx.transform(geometry.orientationTransform);') >
  renderer.indexOf('ctx.scale(geometry.flipScaleX'));
