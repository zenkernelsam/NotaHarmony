...
const importWriter = image.indexOf('async function writeOriginalImageAssetBytesLocked');
const transaction = image.indexOf('await store.beginTransaction()', importWriter);
const saveCall = image.indexOf('await validateNoteReferences(store, noteIds)', transaction);
const insert = image.indexOf("await store.insert('note_asset'", saveCall);
const checks = [
  ['shared validator is exported', helper >= 0],
  ['image import calls shared validator', importWriter >= 0 && transaction > importWriter &&
    saveCall > transaction],
