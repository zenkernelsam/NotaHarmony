...
const checks = [
  ...
  ['publisher validates completion time', source.includes('!Number.isSafeInteger(completedAt)')],
  ['publisher validates source identity', source.includes('source.noteId.length === 0') && source.includes('source.revision.length === 0')],
  ['publisher validates source timestamp and data', source.includes('!Number.isSafeInteger(source.updatedAt)') && source.includes('source.data.length <= 0')],
